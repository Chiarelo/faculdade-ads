function eleicao() {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      while (
        partidos.some((item) => item.codigo == objeto.codigo || objeto.codigo < 0)
      ) {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      while (!partidos.some((item) => item.codigo == objeto.codPartido)) {
        objeto.codPartido = parseInt(
          prompt("Partido não encontrado, informe um novo")
        );
      }
    ...
    for (let i = 0; i < 10; i++) {
      ...
      let aux = politicos.find(
        (item) =>
          item.codPartido == votacao.partido && item.nome == votacao.candidato
      );
      if (aux != undefined) {
        // achou
        aux.qtde++;
